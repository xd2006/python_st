...
def test_delete_contact_from_group(app: Application, db, orm):
    ...
    contacts_in_group = orm.get_contacts_in_group(group)
    assert contact not in contacts_in_group, "Contact id=%s wasn't deleted from group id=%s" % (contact.id, group.id)
